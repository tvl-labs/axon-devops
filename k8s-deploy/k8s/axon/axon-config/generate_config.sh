...
for ((i=0; i<N; i++)); do
    ...
    sed -i "s/bls_pub_key = \"null\"/bls_pub_key = \"$bls_pub_key\"/g" k8s-deploy/k8s/axon/axon-config/chain-spec.toml
    sed -i "s/pub_key = \"null\"/pub_key = \"$pub_key\"/g" k8s-deploy/k8s/axon/axon-config/chain-spec.toml
    sed -i "s/address = \"null\"/address = \"$address\"/g" k8s-deploy/k8s/axon/axon-config/chain-spec.toml
done
...
for ((i=1; i<=N; i++)); do
    ...
    sed -i "s/multi_address = \"\/dns4\/axon-$i\/tcp\/8001\/p2p\/[^\"]*\"/multi_address = \"\/dns4\/axon-$i\/tcp\/8001\/p2p\/$peer_id\"/g" k8s-deploy/k8s/axon/axon-config/node_$i.toml
done